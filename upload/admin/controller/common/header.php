...
class ControllerCommonHeader extends Controller {
	public function index() {
		...
		$data['text_used_product'] = $this->language->get('text_used_product');
		...
		if (!isset($this->request->get['token']) || !isset($this->session->data['token']) || ($this->request->get['token'] != $this->session->data['token'])) {
			...
			$used_product_total = $this->model_catalog_used_staff->getTotalUsedStaffs(array('filter_status' => 0));

			$data['used_product_total'] = $used_product_total;

			$data['used_product'] = $this->url->link('catalog/used_staff', 'token=' . $this->session->data['token'] . '&filter_status=0', true);
			...
			$data['alerts'] = $customer_total + $product_total + $used_product_total + $review_total + $return_total + $affiliate_total;
