...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['token']) && isset($this->session->data['token']) && ($this->request->get['token'] == $this->session->data['token'])) {
			...
			//used staff (custom code)
			if ($this->user->hasPermission('access', 'catalog/used_staff')) {
				$catalog[] = array(
					'name'	   => $this->language->get('text_used_staff'),
					'href'     => $this->url->link('catalog/used_staff', 'token=' . $this->session->data['token'], true),
					'children' => array()		
				);
			}
			//ENDS HERE
